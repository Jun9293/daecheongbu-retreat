...
def test16_s02_이유가_다_있으면_안_걸린다(tmp_path):
    ...
    for 이름 in ("옛말-넘김.txt", "이름-넘김.txt", "이름-확인됨.txt"):
        _넘길것, 안되는줄 = 넘김.읽는다(ROOT / "docs" / 이름)
        assert 안되는줄 == [], f"{이름} 에 넘김이 안 되는 줄: {안되는줄}"
...
def test16_s03_읽는_곳이_하나다():
    ...
    **넘길 것이 없는 검사도 있다** — `check_handoff` 는 보고 맨 위의 모양만 보고
    넘김 목록이 아예 없다(2026-09-11 도막 5). 그것을 **목록으로 빼지 않는다**
    (빼면 다음 검사가 그 목록에 안 적혀 이 축을 조용히 안 지난다) — 코드가
    **`.txt` 를 이름하는가**로 끌어낸다 — 넘김 목록은 전부 `.txt` 이고
    `check_handoff` 가 보는 것은 `.md` 다. 그리고 **양쪽을 다 잰다**:
    목록을 가진 검사는 `넘김.읽는다` 로 읽어야 하고, 안 가진 검사는 `넘김` 을
    아예 안 불러야 한다 — 반쯤 쓰는 자리가 갈리는 자리다."""
    넘김쓰는것 = []
    for 이름 in _load("글검사").검사들:
        ...
        if ".txt" in 코드:
            ...
            assert "넘김" not in 코드, f"{이름} 은 목록이 없는데 넘김을 반쯤 쓴다"
    # ③ 이 시험이 실제로 무언가를 보고 있는가 — 아무도 안 걸리면 0 을 재는 것이다
    assert 넘김쓰는것, "넘김 목록을 가진 검사가 하나도 없다 — 이 시험이 아무것도 안 본다"
...
def test16_s04_넘김_수가_출력에_찍힌다():
    ...
    **넘길 것이 없는 검사는 뺀다** — s03 과 같은 방식으로 코드에서 끌어낸다
    (목록을 여기 적으면 다음 검사가 그 목록에 안 적혀 조용히 안 지난다)."""
    import os
    ...
    for 이름 in _load("글검사").검사들:
        ...
        잰것 += 1
    assert 잰것, "넘김 수를 찍어야 할 검사가 하나도 없다 — 이 시험이 아무것도 안 본다"
