...
def test_w_14_10장이_항목_수를_박아_두지_않는다():
    """전에는 수를 적어 두고 이 시험이 대본과 맞는지 쟀다. 그런데 항목이
    늘 때마다 문서와 시험이 함께 낡았다 — 11-3 의 「문서의 값을 시험에
    박지 않는다」 와 같은 모양이다. 2026-09-06 UI 개편에서 수를 걷어냈다.

    이제 재는 것은 둘이다 — 수가 다시 박히지 않았는가, 그리고 기준
    문장(그때 있는 항목이 전부 통과)이 남아 있는가."""
    text = (ROOT / "CLAUDE.md").read_text(encoding="utf-8")
    at = text.index("## 10. 화면 변경을 끝냈다고")
    section = text[at : text.index("## 11.", at)]
    m = re.search(r"보드 (\d+)항목|달력 (\d+)항목", section)
    assert not m, "10장에 항목 수가 다시 박혔다 — 숫자를 문서에 박지 않는다"
    assert "그때 있는 항목이 전부 통과" in section, "기준 문장이 없다"
