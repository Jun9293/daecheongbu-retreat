...
def test_x_21_시험_자료에_실명이_없다():
    ...
    # **대응표를 여기서 파싱하지 않는다.** 경로도 모양도 묻지 않고
    # `표기들()` 만 부른다 — 두 곳에서 읽으면 구조가 바뀔 때 한쪽만
    # 고쳐진다. 2026-09-04 에 이 시험이 `p[0]` 에서 깨졌다.
...
def test_y_11_무엇으로_골랐는지_화면이_말한다():
    ...
    **낱말만 잰다** — 그 문구가 실제로 뜨는지는 회의록 화면을 부르는
    시험들이 잰다."""
    js = (ROOT / "app" / "static" / "js" / "meeting.js").read_text(encoding="utf-8")
...
def test_w2_01b_화면이_하려는_일을_먼저_크게_그린다():
    ...
    **낱말만 잰다** — 그리는 순서와 어조를 소스에서 본다. 실제로 크게
    보이는지는 브라우저에서만 확인되므로(10장) 판마다 스크린샷으로
    남긴다. 크기 자체는 눈금 시험(test_typescale)이 단으로 지킨다.
    """
    js = (ROOT / "app" / "static" / "js" / "meeting.js").read_text(encoding="utf-8")
    # 하려는 일이 먼저 그려진다
