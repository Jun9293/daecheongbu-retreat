...
def test_f06_머리줄과_시각_열이_본문보다_크고_굵다(sheet_data):
    ...
    # 화면도 같다 — 머리줄은 굵고, 시각 열은 같은 크기(--fs-head)에 본문 글꼴이다
    # (고정폭이 아니다 · UI 정리 판 1 · 3-b)
    css = open("app/static/css/retreat.css", encoding="utf-8").read()
    assert ".c-head > .cx{font-size:var(--fs-head);font-weight:600;" in css
    assert ".c-time > .cx{font-size:var(--fs-head);" in css
