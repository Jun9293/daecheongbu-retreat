...
def test15_d01_4_0_이_하한_셋을_말한다():
    ...
    assert "하한이 셋" in 자리
    # **px 이 아니라 단으로 적는다** (4-0 · 2026-09-21) — `--fz` 를 올리면 px 은
    # 따라 움직이는데 글로 박아 두면 그 순간 낡는다. 이 시험이 px 을 요구하던
    # 시절에는 눈금을 올릴 때마다 여기가 함께 빨개졌다
    assert "보조는 보조 단" in 자리 and "장식은 **장식 단**" in 자리
    assert "px 이 아니라 단으로 적습니다" in 자리
    assert "보조급자리" in 자리
