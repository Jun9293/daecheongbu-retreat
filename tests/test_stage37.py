...
def test37_a02_날짜를_안_바꾸면_업무도_안_움직인다(admin_client, 회차):
    ...
    **② 의 판별력이 어디서 나오는지 적어 둡니다** (2026-09-12 검토가 짚음).
    `update_retreat` 의 「개회일이 바뀌었을 때만」 가드를 떼어도, 라이브러리
    자리에 그대로 있는 run 들은 다시 계산해도 값이 같아 안 걸립니다.
    실제로 걸리는 것은 **날짜 없이 켜진 run 하나**(AW-c 의 그 갈래)가
    D-13 날짜를 얻기 때문입니다. **AW-c 를 고치는 날 이 줄은 조용히
