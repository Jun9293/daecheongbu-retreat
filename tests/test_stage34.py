...
def test34_n01_걷을_것만_걷었다(client):
    ...
    본다 — ① 그 함수가 모듈에 없는가 ② `app/` · `tests/` · `scripts/`
    어디에서도 **부르지 않는가** ③ **뿌리마다 훑은 파일이 비지 않았나**
    (합쳐서 세면 한 뿌리가 통째로 빠져도 다른 뿌리의 수로 통과한다) ④ 함께 남기기로 한 것은 **남아 있고 실제로
    ...
    **찾는 모양은 `이름(` 이다** — 이 독스트링이 그 이름을 설명하려고
    ...
    # **뿌리마다 따로 센다.** 합쳐서 세면 한 뿌리가 통째로 빠져도 다른
    # 뿌리의 수로 통과한다(검토가 짚음). `scripts/` 도 함께 훑는다 —
    # 아래 ④ 가 그 뿌리를 호출면으로 치고 있다
    본파일 = {}
    부르는곳 = []
    for 뿌리 in ("app", "tests", "scripts"):
        본파일[뿌리] = 0
        for path in (ROOT / 뿌리).rglob("*.py"):
            본파일[뿌리] += 1
            if 걷은것 in path.read_text(encoding="utf-8"):
                부르는곳.append(str(path.relative_to(ROOT)))
    for 뿌리, 수 in 본파일.items():
        assert 수 > 10, f"{뿌리} 를 {수}개만 훑었다 — 그 뿌리는 아무것도 안 본 검사다"
