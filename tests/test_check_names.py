...
def test_검사_05_넘긴_낱말은_안_걸리고_홀로_선_이름은_걸린다(tmp_path):
    ...
    """
    C = _검사()
    # 넘긴것() 은 (넘길 것, 안 되는 줄) 이다 — 이유가 비었거나 이유가
    # 그 말을 다시 쓴 줄은 넘기지 않고 부르는 쪽이 실패로 낸다
    # (scripts/넘김.py)
    넘김, 안되는줄 = C.넘긴것()
    assert 안되는줄 == [], f"넘김이 안 되는 줄: {안되는줄}"
...
def test_검사_08_대응표를_읽는_곳이_하나다():
    """**낱말만 잰다** — 다만 「있는가」 가 아니라 **몇 번 나오는가**를
    세는 쪽이라, 자리가 하나 더 생기면 수가 바뀌어 걸린다. 동작(대응표를
    실제로 읽어 오는 것)은 같은 파일의 다른 시험들이 잰다.
