...
def 겹친사본(지키는: set[str], 넘길: set[str]) -> set[str]:
    """지키는판에도 넘김에도 있는 사본 이름 — 둘은 서로 반대라 함께 서면 모순이다."""
    return {f"app-{s}.db" for s in 지키는} & 넘길


def test52_a05_지키는판과_넘김에_함께_있으면_잡는다():
    assert 겹친사본({"20990101-000001"}, {"app-20990101-000001.db", "app-20990101-000002.db"}) == {"app-20990101-000001.db"}
    assert 겹친사본({"20990101-000001"}, {"app-20990101-000002.db"}) == set()
...
def test52_a01_문서가_가리키는_사본은_전부_지키는판이나_넘김에():
    ...
    둘다 = 겹친사본(set(backup.지키는판), _넘김())
    assert not 둘다, f"지키는판에도 넘김에도 있는 사본 — 둘 중 하나에서 빼세요: {둘다}"
