...
def test48_e04_결산_식이_예산_줄_둘에_잡히면_못_잇고_짝_표에(판):
    _결산(판["경로"], 5, "='지출 상세내역'!H6+'지출 상세내역'!H7")   # 지출 7행이 예산 5 · 6행 둘에 잡힌다
    계획 = _돌린다(판, False)
    assert {r: 사유 for r, 사유, _ in 계획.짝["예산"]} == {7: "결산 식이 가리키는 예산 줄이 2개"}
    assert next(e for e in 계획.지출 if e["줄"] == 7)["예산줄"] is None
    assert next(e for e in 계획.지출 if e["줄"] == 6)["예산줄"] == 5
