...
def test13_r02_점검_스크립트가_행_전체를_여는_자리로_본다():
    ...
    **낱말만 잰다** — 점검 스크립트 자신은 브라우저에서만 돌므로,
    여기서는 그것이 **무엇을 재기로 적어 두었는지**만 본다."""
    ...
    # 상태 칸은 **바꿀 수 있을 때만** 안 여는 자리다 (4-14) — 점검도
    # 권한 둘을 나눠 재야 한다. 권한과 무관하게 「안 열려야 한다」 로
    # 적어 두면 관리자로 돌 때만 우연히 통과한다
    assert "열림이 권한과 어긋남" in chk
    assert "메뉴가 권한과 어긋남" in chk
    assert "고를수있나 of [true, false]" in chk
