...
**임시 폴더에 쓰려고 한다.** conftest 가 맨 위에서 `DCB_DATA_DIR` 을 임시 경로로
지정하므로 `app.config.ATTACHMENT_DIR` 도 보통은 그 아래다. **다만 그것은 순서에
기댄다** — app 이 conftest 보다 먼저 읽히면 운영 폴더를 본다. 그때는 DB 도
운영을 가리키므로 세션 첫머리의 검사(`conftest._시험DB가_아니면_멈춘다`)가
시험 전에 멈춰 함께 막히지만, **이 폴더 자체를 재는 검사는 없다** (봐둘것 AZ-a).
네트워크에도 의존하지 않는다.
"""
