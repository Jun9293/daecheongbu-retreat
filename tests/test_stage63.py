...
def test63_b05_본문은_흰_종이이고_구역은_선으로_나뉜다():
    ...
    # **규칙이 먼저 들어오고 쓰는 단계가 뒤인 토큰이 하나 있다.** 0장이 「설계가
    # 바뀌면 코드보다 문서를 먼저 고친다」 이므로, 4-0 에 규칙이 적힌 값은
    # `check_named` 가 실재를 요구한다 — 그런데 그것을 실제로 칠하는 것은 보드
    # 행을 다시 그리는 단계다. 그 사이를 넘기는 자리이고 **이름과 까닭을 함께**
    # 적는다: 목록이 아니라 한 줄이라, 다음에 하나 더 넣으려면 그때 이 글을 읽는다.
    # **비어 있는 것이 맞는 상태다.** `--row-line` 은 2026-09-25 3단계에서
    # 실제로 칠하기 시작해 여기서 뺐다 — 아래 `아직쓰는것` 이 그것을 잰다
    아직안쓰는토큰: dict[str, str] = {}
    죽은토큰 = [이름 for 이름 in re.findall(r"(--[\w-]+)\s*:", 뿌리.group(1))
              if f"var({이름})" not in 부르는데 and f"var({이름}," not in 부르는데
              and 이름 not in 아직안쓰는토큰]
    assert not 죽은토큰, f"부르는 자리가 없는 토큰이 남아 있다: {죽은토큰}"
    # **넘긴 것이 실제로 안 쓰이는 동안만 넘김이다** — 쓰이기 시작하면 이 줄을
    # 지워야 다음 사람이 「아직 안 쓴다」 를 사실로 읽는다
    아직쓰는것 = [이름 for 이름 in 아직안쓰는토큰
               if f"var({이름})" in 부르는데 or f"var({이름}," in 부르는데]
    assert not 아직쓰는것, f"이제 쓰는데 넘김에 남아 있다: {아직쓰는것}"
