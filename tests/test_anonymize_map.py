...
def test_대응표_04_표기와_가명의_글자_수가_같다():
    ...
    **이 규칙은 여러 곳에 적혀 있었는데 보는 시험이 하나도 없었다** —
    CLAUDE.md 11-2 · `anonymize.py` 머리말 · 표기별 가명 주석.
    그래서 3자 표기 하나가 2자 가명을 쓰고 있는 것을 아무도 못 봤다.

    **장소는 이 규칙 밖이다** (11-2) — 규칙의 이유(화면 폭·`M` 접미)가
    둘 다 사람 이름의 사정이다. 장소 가명은 길이가 달라도 된다.
    """
    A = _익명()
    장소표기 = {a for a, _ in A.장소들()}
    안맞 = [(t, A.가명(t)) for t in A.표기들()
          if t not in 장소표기 and A.가명(t) and len(t) != len(A.가명(t))]
    가려서 = [(t[0] + "◯" * (len(t) - 1), f) for t, f in 안맞]
    assert not 안맞, f"표기와 가명의 글자 수가 다르다: {가려서}"


def _글자수_안맞는_사람_표기(names, 장소표기: set) -> list:
    """사람 표기 중 가명과 글자 수가 다른 것 — 장소는 밖이다."""
    return [(t, f) for t, f in names
            if t not in 장소표기 and len(t) != len(f)]


def test_대응표_04b_길이_다른_장소_쌍은_통과한다():
    """막히는 쪽과 뚫리는 쪽 (11-3) — 규칙이 장소에 안 걸리는 것을
    합성 자료로 잰다. 진짜 대응표의 장소가 우연히 같은 길이여도
    이 시험은 규칙 자체를 재므로 낡지 않는다."""
    names = [("가상의긴장소이름", "짧은곳"), ("두글", "세글자")]
    장소표기 = {"가상의긴장소이름"}
    안맞 = _글자수_안맞는_사람_표기(names, 장소표기)
    assert 안맞 == [("두글", "세글자")]     # 사람 쪽만 걸린다 — 장소는 통과
