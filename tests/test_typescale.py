...
# **눈금이 이미 본문 단에 두기로 한 것들.** 4-0 의 표가 본문 단을
# 「body·입력·논의·왼쪽 라벨」 로 적었으므로, 입력칸과 표가 그 단인 것은
# **올린 것이 아니라 제자리**다. 단추는 여기 안 든다 — 크게 둘지 말지가
# 그 화면의 판단이라, 그것이야말로 이름으로 적어 둘 자리다.
...
올린자리 = {
    ...
    ".kv":
        "두 칸 정의 표의 본문 — 이름·연락처·부서 같은 값이 그 자리에 선다. "
        "값이 곧 내용이라 목록 단이 맞고, 올린 것이 아니라 원래 그 급이다. "
        "왼쪽 라벨(.kv .k)은 옆의 값이 뜻을 지므로 한 급 아래다 (4-9)",
    ".loginform button:not(.sbtn)":
        "로그인 화면의 단추 — 그 화면에서는 이것 하나가 할 일의 전부라 "
        "본문보다 크다. 설정 페이지의 단추는 `sbtn` 을 밝혀 이 규칙을 "
        "비켜 가므로(2026-09-12 에 사람이 정함) 여기 든 것은 로그인 쪽뿐이다",
}


def _설정화면의_클래스() -> set[str]:
    """설정 페이지가 실제로 쓰는 클래스 이름. **화면에서 읽습니다** —
    손으로 적으면 칸이 하나 늘 때 목록만 낡습니다(10장)."""
    글 = (ROOT / "app" / "templates" / "settings.html").read_text(encoding="utf-8")
    쓰는것: set[str] = set()
    for m in re.finditer(r'class="([^"{}]+)"', 글):
...
def test_ts_02c_급을_올린_자리가_목록에_있다():
    ...
    범위는 **설정 페이지 전체**입니다. 처음에는 `.fld`(카드 하나)만 봐서
    **다른 카드에서 급을 올리면 안 걸렸습니다** — 2026-09-12 에 사람이
    넓히라고 정했습니다.

    「이 페이지에 걸리는 규칙인가」 는 **화면이 쓰는 클래스**로 가릅니다 —
    선택자에 나오는 클래스가 전부 그 화면에 있으면 이 페이지의 것입니다.
    `.calundated .hint` 처럼 같은 `.hint` 를 쓰지만 다른 화면의 규칙은
    이 물음의 대상이 아닙니다.

    봅니다 — ① 적어 둔 자리가 실제로 본문 단(14px) 이상인가 ② 까닭이
    적혀 있는가 ③ **CSS 에서 끌어낸 것이 목록 안에만 있는가** — 이 페이지
    어느 카드에 둘째가 생겨도 걸립니다 ④ `.hint` 는 장식 단 그대로인가
    ⑤ **다른 화면의 규칙까지 끌어오지는 않았나**(그러면 이 목록이 설정과
    상관없는 이름으로 부풀어 뜻을 잃습니다) ⑥ 실제로 읽은 것이 있나.

    **제목과 입력칸·표는 이 물음의 대상이 아닙니다** — 4-0 의 눈금이 이미
    그 급에 두기로 한 것들이라, 거기 있는 것은 올린 것이 아니라 제자리입니다.
    **단추는 대상입니다** — 크게 둘지는 그 화면의 판단이고, 이 판이 고친
    것이 정확히 그 자리입니다.
    """
    본문 = 단.index("--fz-md")
    for sel, 이유 in 올린자리.items():
        assert 단번호(sel) >= 본문, f"① {sel} 이 본문 단(14px)에 못 미친다"
        assert len(이유) > 30, f"② {sel} 의 까닭이 비었다"
    assert 단번호(".fld .hint") < 본문, "④ .hint 가 함께 올라갔다"

    쓰는클래스 = _설정화면의_클래스()
    assert {"fld", "hint", "pwwarn"} <= 쓰는클래스, "⑥ 화면을 못 읽었다"
    ...
    for m in re.finditer(r"([^{}]+)\{([^{}]*)\}", _민낯()):
        ...
        for sel in (" ".join(x.split()) for x in m.group(1).split(",")):
            ...
            # ⑤ 선택자의 클래스가 **전부** 이 화면의 것일 때만 이 페이지의 규칙이다
            if not 이름 or not 이름 <= 쓰는클래스:
                continue
            # **제목은 이 물음의 대상이 아닙니다** — 4-0 의 눈금이 제목 급을
            # 따로 두었고, 카드 제목이 본문보다 큰 것은 올린 것이 아니라
            # 원래 그 급입니다
            마지막 = sel.rsplit(" ", 1)[-1]
    ...
    assert 본것 >= 4, f"⑥ 설정 페이지의 자리를 {본것}개만 읽었다 — 검사가 새고 있다"
    assert 올린것 <= set(올린자리), (
        f"③ 목록에 없는 자리가 급을 올렸다: {sorted(올린것 - set(올린자리))}")
    assert set(올린자리) <= 올린것, (
        f"③ 목록에만 있고 CSS 에 없는 자리: {sorted(set(올린자리) - 올린것)}")
