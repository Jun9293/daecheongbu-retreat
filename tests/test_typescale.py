...
def test_ts_01b_눈금이_하나에서_나온다():
    뿌리 = 선언(":root")
    assert "--fz:16px" in 뿌리.replace(" ", ""), "기준 크기가 없다"
    for 이름 in 단[:1] + 단[1:]:
        if 이름 == "--fz-base":
            continue
...
def test_ts_02_본문_목록_사이드바가_하한_아래로_안_내려간다():
    """혼자 뜻을 지는 글자의 하한은 **목록 단**(`--fz-md`)이다.

    **px 으로 적지 않는다** — `--fz` 를 올리면 px 은 따라 움직이는데 글로 박아
    두면 그 순간 낡는다 (4-0 · 2026-09-21 에 한 번 그랬다).
    """
    하한 = 단.index("--fz-md")
    자리 = {
        "body": "본문",
    ...
    assert not 작은것, f"목록 하한(--fz-md) 아래인 곳: {작은것}"


# **보조 급의 자리** — 옆이나 아래가 뜻의 절반을 지는 조작·메타.
# 하한은 보조 단이고 그 아래로는 안 내려간다. 4-0 의 눈금이
# 보조를 「메타·날짜·작성자·부속 숫자」 로 두었으므로, 그 성격의 자리는
# 여기서 잰다 — **장식 단으로 흘러내리지 않게 하는 것**이 목적이다.
보조급자리 = {
    ".tlist .lsort .chip":
        "목록의 정렬 토글 — 누르면 그 아래 목록이 곧바로 그 순서가 되고, "
        "지금 축과 방향은 화살표가 보인다. 무엇을 볼지(상태 칩은 목록 단)가 "
        "먼저고 순서는 그다음이라 한 급 아래다 (4-14)",
    ".lc .pup":
        "보드 왼쪽 칸의 상위 표시 — 머리 줄로 선 하위에 「↑ 상위 제목」 을 붙인다. "
        "옆의 업무명(목록 단)이 뜻의 주인이고 이 줄은 그 업무가 무엇의 하위인지를 "
        "보탠다 (봐둘것 BF-a · 2026-09-17 사람이 정함)",
    ".trow.lrow .pup":
        "목록 행 메타의 상위 표시 — 부서 칩 · 담당자 · 마감 옆에 선다. 누르면 "
        "상위의 상세가 열린다. 행의 제목이 뜻의 주인이다 (봐둘것 BF-a)",
    ".mrow .meta .pup":
        "좁은 폭 보드 목록의 상위 표시 — 같은 줄의 부서 · 날짜 메타(보조 단)와 "
        "한 급이다. 위의 업무명이 뜻의 주인이다 (봐둘것 BF-a)",
    # 예산 표의 항목 라벨은 2026-09-26 에 없어졌다 — 항목이 **자기 칸으로
    # 병합**되면서 세부항목 줄과 같은 급이 됐다(7-3 확정본). 급을 낮출 자리가
    # 아니라 자리 자체가 사라진 것이라 여기서 뺀다
}
...
def test_ts_02b_보조_급_자리가_장식_단으로_안_내려간다():
    ...
    for sel, 이유 in 보조급자리.items():
        assert 단번호(sel) == 보조, f"{sel} 이 보조 단이 아니다"
        assert len(이유) > 30, f"{sel} 의 이유가 비었다"
        assert 단번호(sel) < 단번호(".chip"), f"{sel} 이 상태 칩보다 작지 않다"


# **최소 누름 크기의 자리** — 글자는 보조 급으로 작아도 **누르는 칸**은 24px 아래로
# 안 내려간다(흔히 쓰는 최소 누름 크기 · 사람이 정함 2026-09-17 · 봐둘것 BF-a).
# 글자 크기와 따로 재는 까닭은 급을 낮춘 자리가 누르기까지 어려워지면 안 되기 때문이다.
누름최소 = 24
누름자리 = {
    ".trow.lrow button.pup":
        "목록 행의 「↑ 상위」 단추 — 휴대폰에서도 누르는 단추인데 글자가 보조 급이라 "
        "줄 높이만으로는 21px 남짓이다. 위아래 여백으로 칸을 키운다",
}


def _선언들(sel: str) -> list[str]:
    """그 선택자가 **정확히** 든 규칙 전부 — 한 선택자가 여러 규칙에 나뉘어 있다."""
    return [m.group(2) for m in re.finditer(r"([^{}]+)\{([^{}]*)\}", _민낯())
            if sel in [" ".join(x.split()) for x in m.group(1).split(",")]]


def test_ts_02d_누르는_칸이_최소_크기_아래로_안_내려간다():
    for sel, 이유 in 누름자리.items():
        assert len(이유) > 30, f"{sel} 의 이유가 비었다"
        높이 = [int(m.group(1)) for d in _선언들(sel)
                for m in re.finditer(r"min-height:\s*(\d+)px", d)]
        assert 높이, f"{sel} 에 min-height 가 없다 — 누르는 칸 크기를 안 정했다"
        assert max(높이) >= 누름최소, f"{sel} 의 누르는 칸이 {max(높이)}px 이다"
...
올린자리 = {
    ...
    ".fld .hint.lone":
        "설정의 `.hint` 중 **혼자 뜻을 지는 넷** — 회차 상세의 「개회일을 바꾸면 "
        "업무 날짜가 함께 움직인다」 · 바깥 링크의 「한쪽만 비어도 항목이 "
        "사라진다」 · 부서의 「여기서 만든 부서는 이 회차에만 붙는다」 · 내 "
        "정보의 「푸시로 무엇이 오는가」. 넷 다 옆에 뜻을 대신 져 줄 값이 없다. "
        "**새 규칙을 판 것이 아니라 나머지와 맞춘 것이다** — `.hint` 규칙은 "
        "CSS 에 여섯인데 설정의 것만 장식 단이었고 다른 다섯은 보조~목록 단이다 "
        "(2026-09-12 에 사람이 정함). `.pwwarn` 과 안 합친다 — 그쪽은 색과 "
        "굵기까지 올린 자리다",
}
...
def test_ts_02c_급을_올린_자리가_목록에_있다():
    ...
    봅니다 — ① 적어 둔 자리가 실제로 본문 단 이상인가 ② 까닭이
    ...
    **단추와 지표 숫자는 안 거릅니다** — 크기를 고를 여지가 있는 자리라
    ...
    for sel, 이유 in 적은것.items():
        assert 단번호(sel) >= 본문, f"① {sel} 이 본문 단에 못 미친다"
...
def test_ts_03_장식_단_아래로_내려가는_단이_없다():
    """가장 작은 단이 `--fz-xs` 이고, 그보다 작은 단은 없다.

    **이름에도 px 을 안 적는다** — 함수 이름은 pytest 출력에 그대로 뜨므로,
    눈금을 올린 날 그 줄이 거짓말을 한다 (4-0).
    """
    뿌리 = 선언(":root").replace(" ", "")
    assert "--fz-xs:calc(var(--fz)-3px)" in 뿌리, "가장 작은 단이 눈금에서 안 나온다"
...
def test_ts_04_위계가_유지된다():
    ...
    # 그룹은 잉크 600, 하위는 보조색. 둘 다 목록 하한(--fz-md)이다.
...
def test_ts_06_칸_높이도_함께_올렸고_이유가_적혔다():
    """바 안에 목록 단 글자가 들어가는데 높이가 그대로면 위아래가 붙는다.

    **재서 판단한 것을 남긴다** — 다음 사람이 "왜 이 값인가" 를 물을 자리다.
    """
    뿌리 = 선언(":root").replace(" ", "")
    assert "--h-main:40px" in 뿌리 and "--h-sub:35px" in 뿌리 and "--h-team:44px" in 뿌리
    assert ".bar.m{height:28px}" in CSS.replace(" ", "")
    # 왜 이 값인지가 주석에 있다 (숫자와 함께)
    at = CSS.index("--h-team:44px")
...
흐려도_되는곳 = {
    ...
    ".mt-one-meta label",
    # 예산 표의 이동 손잡이 — 글자가 아니라 **끌 자리를 가리키는 표시**이고,
    # 그 줄의 이름·값이 뜻을 다 진다. 편집을 켰을 때만 뜬다 (7-3 · 2026-09-26)
    ".budtbl.editing .grip",
    ...
    # 뺀 업무 접힘 캐럿(▸) — 같은 자리다. 옆의 「이번 회차에서 뺀 업무 N건」 이 뜻을 진다
    ".lexcl > summary::before",
    ...
    # 목업 확정분 1단계 (2026-09-23) — 드로어와 그 팝업들.
    # 셋 다 **옆의 값이 뜻을 다 지는 자리**다: 꼬리표는 그 옆의 팀 이름이,
    # 결과 표의 왼쪽 라벨은 오른쪽 값이(`.dmeta dt` 와 같은 자리), 캐럿은
    # 접혔는지를 말하는 기호일 뿐 판정 이름이 뜻을 진다.
    ".plist.notify .tag",                                 # 「기존」·「새로 추가」 — 옆이 팀 이름
    ".relres dt",                                         # 결과 세 줄의 왼쪽 라벨
    ".dg-h .caret",                                       # 진단 한 줄의 ▼/▲
    # 목업 확정분 2단계 (2026-09-25) — 업무 추가 팝업 (6-7)
    ".calplus",                          # 날짜 칸의 「+」 — 글자가 아니라 기호이고,
                                         # 무엇에 더하는지는 그 칸의 날짜가 진다 (4-13)
    ".addpop .apbar .apsub",             # 윗줄의 부제 — 옆의 「업무 추가」 가 뜻을 지고,
                                         # 목업 C 가 「흐린 부제」 로 적은 그 자리다
}
