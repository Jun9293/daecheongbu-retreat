...
def test_ts_02c_급을_올린_자리가_목록에_있다():
    ...
    적혀 있는가 ③ **CSS 에서 `.hint` 보다 큰 자리가 이 목록 안에만
    있는가** — 같은 카드에 둘째가 생기면 여기 안 적히고 지나갑니다.
    ④ `.hint` 자체는 장식 단 그대로인가(올린 것이 한 줄이라는 뜻).
    """
    본문 = 단.index("--fz-md")
    ...
    # ③ **CSS 에서 끌어내 목록과 견줍니다** — 손으로 적은 것끼리 견주면
    #    둘째 자리가 생겨도 목록에 없으니 아무 일도 안 일어납니다
    본것, 올린것 = 0, set()
    for m in re.finditer(r"([^{}]+)\{([^{}]*)\}", _민낯()):
        ...
        for sel in (" ".join(x.split()) for x in m.group(1).split(",")):
            ...
            # 원래 그 급입니다. 보는 것은 **본문 자리의 글**뿐입니다
            if not sel.startswith(".fld ") or sel.rsplit(" ", 1)[-1] in 제목태그:
                continue
            본것 += 1
            if 단.index(크기.group(1)) >= 본문:
                올린것.add(sel)
    assert 본것 >= 2, "③ `.fld` 자리를 못 읽었다 — 검사가 볼 것을 안 보고 있다"
    assert 올린것 <= set(올린자리), (
