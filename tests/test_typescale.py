...
# **장식 단(12px)에 두는 자리는 이름으로 적는다.** 「옆의 값이 뜻을 다
# 지는가」 는 기계가 못 가르므로, 4-0 의 흐림 자리를 이름으로 적어 둔 것과
# 같은 방식이다 — 여기 없는 곳이 12px 로 내려가면 test_ts_02 의 하한에
# 걸리게 두고, 여기 있는 것은 **왜 그래도 되는지**를 함께 적는다.
장식이어도_되는곳 = {
    ".tlist .lsort .chip":
        "정렬 토글 — 누르면 그 아래 목록이 곧바로 그 순서가 된다. "
        "무엇을 볼지(상태 칩)가 먼저고 순서는 그다음이라 한 급 아래다 (4-14)",
}


def test_ts_02b_장식_단은_적어_둔_자리만이다():
    """12px 로 내려간 자리가 목록에 적혀 있는가 — 없이 내려가면 test_ts_02
    의 하한에 걸린다. 이 시험은 **적어 둔 것이 실제로 그 크기인지**를 본다
    (③ 검사가 볼 것을 보고 있나 — 11-3)."""
    for sel, 이유 in 장식이어도_되는곳.items():
        assert 단번호(sel) == 단.index("--fz-xs"), f"{sel} 이 장식 단이 아니다"
        assert len(이유) > 20, f"{sel} 의 이유가 비었다"
