...
| 잰 것 | 어디 |
|---|---|
| 가·나 | 들여오기가 멈출 때 찍는 줄 번호가 시트의 행 번호와 같은가 |
...
def test31_a01_같은_줄이_앞에_두_벌_있어도_멈춘_줄의_번호를_말한다(들여오기, tmp_path):
    ...
    본다 — ① 멈추는가 ② 말에 나오는 수가 **멈춘 그 줄**의 시트 행 번호인가
    ...
    with pytest.raises(들여오기.멈춤) as e:
        들여오기.줄뽑기("총무", p)
    말 = str(e.value)
    assert f"시트의 {멈출자리}행" in 말, f"멈춘 줄의 번호가 아니다: {말}"
...
# ── 나. 그 번호가 시트에서 사람이 보는 행 번호와 같다 ────────────────────

@pytest.mark.parametrize("메모줄수", [0, 1, 4])
def test31_a02_머리글이_첫_줄이_아니어도_시트의_행_번호를_말한다(들여오기, tmp_path, 메모줄수):
    """나) 머리글 위의 메모 줄 수를 0·1·4 로 바꿔 가며 잰다.

    본다 — 찍히는 수가 **파일에서 그 줄이 실제로 몇 번째인가**와 늘 같은가.
    머리글 자리가 움직여도 따라 움직이면 「시트에서 사람이 보는 행」 이 맞다.
    ...
    with pytest.raises(들여오기.멈춤) as e:
        들여오기.줄뽑기("총무", p)
    assert f"시트의 {기대}행" in str(e.value)
...
def test31_a05_칸_안에_줄바꿈이_있어도_행_번호가_안_밀린다(들여오기, tmp_path):
    ...
    본다 — ① 비고에 줄바꿈이 든 칸(시트가 따옴표로 감싼 꼴)을 넣고 ② **시트에서
    사람이 보는 행 번호**를 손으로 적어 둔 값과 견준다. 판독기에서 기대값을
    ...
    # 시트에서 사람이 보는 행: 1 머리글 · 2 볼펜(비고가 두 줄짜리) · 3 자국 줄
    with pytest.raises(들여오기.멈춤) as e:
        들여오기.줄뽑기("총무", p)
    assert "시트의 3행" in str(e.value), f"줄바꿈 때문에 번호가 밀렸다: {e.value}"
