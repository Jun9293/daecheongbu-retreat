...
def test66_d03_깨끗한_판을_맨_처음과_맨_끝에_잰다():
    """**「안 심음」 칸을 걷고 그 자리를 깨끗한 판 둘이 맡는다**
    (2026-09-24 사람이 정함 — 앞 판 검토가 짚은 자리).

    그 칸은 갈래마다 다시 재는 것이 아니라 **맨 위 깨끗한 판을 옮겨 적은
    것**이라 늘 같은 값이었다 — 두 칸처럼 보이지만 잰 것은 하나였다.
    이제 안 심었을 때 깨끗한지는 **맨 처음과 맨 끝** 판이 말하고, 맨 끝이
    빨가면 걷히지 않은 고장이 있다는 뜻이라 **판정 전체가 무효**다.
    """
    assert "안심음" not in 점검JS, "걷기로 한 칸이 남아 있다"

    자리 = 점검JS[점검JS.index("**한 갈래는 한 줄이고"):][:1400]
    assert "걸렸나: 잡혔나" in 자리, "걸렸나 칸이 없다"
    assert "걸린초" in 자리, "갈래마다 걸린 시간을 안 낸다"
    # **짝을 표에 또 싣지 않는다** — `나와야` 하나가 든다
    assert "나와야:" not in 자리, "짝을 표에도 적어 두 벌이 됐다"

    # 맨 끝 판과, 그것이 판정을 무효로 만드는 길
    assert "const 끝깨끗 = await 점검();" in 점검JS, "맨 끝 깨끗한 판이 없다"
    # **식 전체를 글자로 박지 않는다** — `다걷혔나` 가 **첫 자리**여야만
    # 통과하면, 조건을 하나 앞에 끼우는 것만으로 회귀가 아닌데 빨개진다.
    # **떼어내는 코드를 여기 다시 적지 않는다** — 같은 것이 두 곳에 있으면
    # 갈린다. `test_stage19._판정식()` 이 그 하나다 (2026-09-24 커밋 전 검토)
    from tests.test_stage19 import _판정식
    assert "다걷혔나" in _판정식(), "맨 끝 판이 판정에 안 걸린다"
    assert "맨처음:" in 점검JS and "맨끝:" in 점검JS, "두 판을 나란히 안 낸다"
