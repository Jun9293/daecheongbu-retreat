...
# **`배` 도 뺀다** — 「그 자리가 40배가 된다」 는 자리를 센 것이 아니라
# **곱절**이다. 파일째 넘기면 그 파일에 앞으로 쌓이는 것까지 다 밖이
# 되므로(2026-09-24 커밋 전 검토) 무늬를 좁히는 쪽으로 고쳤다
센말뒤 = re.compile(rf"(?:곳|자리|군데)(?:은|는|이|가)\s*({수})(?![\d\-배])")
