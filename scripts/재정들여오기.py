...
def 고른다(db: Session, retreat: Retreat, 판: 시트, 짝표: dict) -> 계획:
    ...
    for e in 판.지출:
        수["지출"] += 1
        # 예산 항목 잇기 (시트-나 ㄱ) — 짝 표가 이긴다
        key = (e["구분"], e["항목"], e["세부1"])
        if e["줄"] in 짝표["예산"]:
            e["예산줄"] = 짝표["예산"][e["줄"]]
            수["지출 · 예산 항목 짝 표로 이음"] += 1
        elif 열쇠.get(key) == 1:
            e["예산줄"] = next(b["줄"] for b in 판.예산 if (b["구분"], b["항목"], b["세부"]) == key)
            수["지출 · 예산 항목 글자로 이음"] += 1
        else:
            e["예산줄"] = None
            사유 = "같은 글자의 예산 줄이 둘 이상" if 열쇠.get(key, 0) > 1 else "같은 글자의 예산 줄이 없음"
            수[f"지출 · 예산 항목 못 이음({사유})"] += 1
            짝["예산"].append((e["줄"], 사유, key))
        if e["예산줄"] is not None and e["예산줄"] not in 줄로:
            raise 멈춤(f"짝 표의 예산 줄 {e['예산줄']} 이 시트의 예산 줄이 아닙니다.")
