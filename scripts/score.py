# -*- coding: utf-8 -*-
"""2판 채점표(`data/제안-2판.real.md`)를 **한 화면에 하나씩** 보여주는 작은 서버.
...
누를 때마다 `data/제안-2판.real.md` 에 바로 쓴다. 화면은 그 파일을 매번
...
그래도 **쓰기 전에 원본을 백업한다** (`data/제안-2판.real.<시각>.bak`).
27개는 Claude 가 낸 것이라 다시 내면 같은 것이 나오지 않는다.
"""
...
def 백업한다(길: pathlib.Path) -> pathlib.Path:
    """**첫 쓰기 전에 한 번.** 파일을 망가뜨리면 27개를 다시 낼 수 없다 —
...
/* **머리와 발은 늘 보인다** — 어디까지 했는지와 누를 단추가 스크롤로
   사라지면, 27개를 넘기는 동안 매번 아래로 내려가야 한다. 그래서 창을
...
function drawLeft(g){
  ...
  if(g.갈래==='제안'){
    ...
    p.innerHTML='<h2>놓친 것</h2>'
      +'<div class="what">마땅히 나왔어야 하는데 안 나온 것이 있나요?</div>'
      +'<div class="why">이 회의의 제안을 다 봤습니다. 이 칸이 없으면 '
      +'<b style="display:inline">적게 내는 쪽으로 점수를 올릴 수 있습니다.</b></div>';
  }else{
...
function summary(전부){
  ...
  let h='<table><tr><th>종류</th><th>1판 (낱말)</th><th>2판 — O</th>'
  ...
  if(전부){
    h+='<table><tr><th>X 이유</th><th>2판</th></tr>';
...
def main() -> None:
    ap = argparse.ArgumentParser(description="2판 채점표를 한 화면에 하나씩 채운다")
