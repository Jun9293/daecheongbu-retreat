...
**이 길은 안 씁니다** (2026-09-16 에 사람이 정했습니다) — 짝짓기·등급·확인용 파일 대신 노션 업무를
그대로 옮겨 놓는 쪽으로 갈아탔습니다(`scripts/노션업무들이기.py` · 봐둘것 BC-c). 지우지 않고 남겨 둡니다:
다시 짝을 지을 날이 오면 여기서 시작합니다. 아래 설명은 그때의 규칙 그대로입니다.
...
- 두 파일이 같은 것을 말하는지는 `견준다` 가 봅니다 — 엑셀에서 꺼낸 표시가 `.md` 와 같은 답이어야 합니다
