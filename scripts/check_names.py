...
def 미확인이미지(목록: list[tuple[str, str]], 확인글: str) -> list[str]:
    """확인 파일의 **같은 줄**에 경로와 지금 내용의 해시가 함께 안 적힌 이미지.

    순수 함수라 시험이 바로 잰다. 해시가 다르면 — 확인한 뒤 내용이
    바뀌었으면 — 확인 안 된 것이다. 파일 어디든 있으면 통과로 두면
    두 이미지의 해시를 맞바꾼 것이 지나간다 (W-a) — 줄 단위로 짝을 본다.
    """
    줄들 = 확인글.split("\n")
    def 확인됨(상대: str, 해시: str) -> bool:
        return any(상대 in 줄 and 해시 in 줄 for 줄 in 줄들)
    return [f"{상대} (해시 {해시})" for 상대, 해시 in 목록 if not 확인됨(상대, 해시)]
...
def main() -> int:
    ...
    # **조건을 못 채운 줄은 넘김이 아니다** (scripts/넘김.py) — 이유가
    # 비었거나 이유가 그 말을 다시 쓴 줄. 조용히 버리면 적은 사람은
    # 넘긴 줄 알고 검사는 안 넘긴 채로 돈다
    넘길낱말, 안되는줄 = 넘긴것()
    if 안되는줄:
        넘김.말한다(넘긴목록, 안되는줄)
        return 2
