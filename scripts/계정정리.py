...
- **`activity_logs.target_id` 는 건드리지 않습니다 — 두기로 했습니다
  (2026-09-10).** `target_type='user'` 인 행이 지운 계정의 id 를 가리킨 채
  남습니다(2026-09-09 실측 8행). 비우면 「무엇에 대한 기록인가」 를 잃는데,
  `summary` 에 그때의 이름이 실려 있어 **두어도 읽힙니다.** 없는 id 를
  가리키는 것은 그 기록이 「지운 계정에 대한 것」 이었다는 사실 그 자체라
  거짓이 아닙니다.
