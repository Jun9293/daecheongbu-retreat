...
- **점수를 다 매긴 뒤 높은 것부터 확정합니다** — 앞에서 먹은 짝이 뒤의 더 맞는 짝을 안 밀어냅니다(전역에 가깝게).
  같은 자료면 늘 같은 답이고, **점수가 정확히 같을 때만** 줄 번호가 이깁니다
...
def 표줄(글: str) -> list[str]:
    """확인용 표의 **자료 줄**만 — 머리 · 가름줄 · 본문은 뺀다(번호로 시작하는 줄이 자료다)."""
    난것 = []
...
def 손댄줄(옛글: str, 새글: str) -> list[str]:
    """**사람이 손댄 줄**을 돌려준다 — 새로 낸 판에 없는 자료 줄이 곧 사람이 적은 줄이다.

    칸 수로 「답 칸이 어디인가」 를 세지 않는다 — 편집기가 끝 빈 칸을 하나 떨어뜨리면
    그 줄이 통째로 안 보여, 답이 적힌 줄을 덮는다(커밋 전 검토 [M] 5).
    새 판과 글자로 견주면 칸이 늘든 줄든 「달라졌다」 가 그대로 남는다.
    """
    새것 = set(표줄(새글))
    return [줄 for 줄 in 표줄(옛글) if 줄 not in 새것]
