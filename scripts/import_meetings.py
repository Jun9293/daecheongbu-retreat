...
def mark_highlights(body: str) -> str:
    """본문을 **사람이 읽을 수 있게** 정리한다.

    ## 형광펜 — 사실만 남기고 뜻은 안 붙인다

    처음에는 `⟨미완료?⟩` 였는데, 물음표가 있어도 뜻을 주장한다 — 나중에 읽는
    사람은 물음표를 안 읽고 "미완료" 만 읽는다. 근거는 2/20 이다
    (`meeting_import` 의 머리말). 칠해져 있었다는 것은 **사실**이고 뜻은
    모른다. 사실만 남긴다 (6-9).

    노란 형광펜은 노션에 **두 가지 모양**으로 적혀 있다. 하나만 처리하고
    있었더니 안내문은 `⟨형광펜⟩` 을 설명하는데 본문에는 `{color="yellow_bg"}`
    가 그대로 남아, **안내문과 본문이 어긋났다.**

        <span color="yellow_bg">…</span>      25곳
        … {color="yellow_bg"}                  3곳   ← 놓치고 있었다

    ## 빨간 형광펜은 노란 것과 **다르게** 적는다

    빨간 형광펜(`red_bg`)이 19곳 있다. **노란 것과 뜻이 같은지 다른지 모른다.**
    쓰인 것을 보면 `면장갑`(라텍스에서 바꾼 결정), `수련회 간략광고 6/7`(날짜),
    `아날로그 예배라서 주보광고로 진행`(방식) — 결정처럼 보이는 것도 있고
    그냥 눈에 띄게 칠한 것도 있다.

    **모르면 합치지 않는다.** 합치면 두 색이 같은 뜻이었다는 주장이 되고,
    그 주장은 아무도 한 적이 없다. `⟨빨간형광펜⟩` 으로 따로 적어 **원본의
    구별을 그대로 지킨다** (6-9). 뜻이 밝혀지면 그때 정한다.

    ## 나머지 태그는 걷어낸다

    `<span color="red">` · `<details>` · `<summary>` 가 본문에 그대로 남아
    있었다. **읽기 어려운 것도 문제지만 그것만이 아니다** — 제안이 낱말
    겹침으로 도는데 `span` · `color` · `red_bg` 가 낱말로 세어진다.
    """
    import re

    # ① 형광펜 — 두 모양 다. 색마다 다른 표시를 붙인다
    for 색, 표 in (("yellow_bg", "⟨형광펜⟩"), ("red_bg", "⟨빨간형광펜⟩")):
        body = re.sub(rf'<span color="{색}">(.*?)</span>',
                      lambda mm, 표=표: f"{표}{mm.group(1).strip()}",
                      body, flags=re.S)
        # `내용 {color="yellow_bg"}` — 뒤에 붙는 모양. 줄 앞에 표시를 단다
        body = re.sub(rf'^(\s*)(.*?)\s*\{{color="{색}"\}}\s*$',
                      lambda mm, 표=표: f"{mm.group(1)}{표}{mm.group(2).rstrip()}",
                      body, flags=re.M)

    # ② 나머지 색 태그는 걷어낸다 (뜻이 없거나 우리가 모르는 것)
    body = re.sub(r'<span color="[a-z_]+">|</span>', "", body)
    body = re.sub(r'\s*\{color="[a-z_]+"\}', "", body)

    # ③ 접힌 덩어리 — 들여쓰기가 이미 구조를 말해 주므로 껍데기만 벗긴다
    body = re.sub(r"</?details>\s*\n?", "", body)
