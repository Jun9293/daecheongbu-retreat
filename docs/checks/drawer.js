...
const 자가시험 = async () => {
  ...
  for (const g of 고장들) {
    ...
    if (못심는까닭) {
      ...
      표.push({갈래: g.갈래, 심은고장: g.고장, 안심음: '—', 심음: 못심는까닭,
               잡았나: null});
      continue;
    }
    ...
    if (!잡혔나) 놓친갈래++;
    /* **두 칸을 적는다** (2026-09-24 사람이 정함) — 「안 심음 통과」 와
       「심음 실패」 가 **둘 다 맞을 때만** 잡은 것이다. 짝이 틀리면 심어도
       ✗ 가 안 나거나(놓침) 안 심어도 ✗ 가 있어(거짓 잡음) 둘 중 하나가 깨진다.
       「안 심음」 은 맨 위의 깨끗한 판이 잰 것이다 — 그 판은 실패가 0 이므로
       **이 항목도 그때 통과**했다. 그래서 갈래마다 다시 안 돌린다. */
    표.push({갈래: g.갈래, 심은고장: g.고장,
             안심음: 깨끗.실패.length === 0 ? '통과' : '(깨끗한 판이 이미 빨감)',
             심음: 잡혔나 ? (g.건너뜀나와야 ? '건너뜀 — 잡음' : '✗ — 잡음')
