...
/* **묶임은 판마다 재지 않고 한 번만 잰다** (2026-09-24 커밋 전 검토).
   ...
   늘린다** — 줄이려고 연 물음에 시간을 더하는 꼴이다. 그리고 쓰이는 값은
   첫 판 것 하나뿐이었다. */
let 묶임잰것 = null;
const 잠깐잔다 = ms => new Promise(r => setTimeout(r, ms));

/* **타이머가 묶였는지 재어 말한다** (2026-09-24 · 지시문 3 · 봐둘것 BI-h).
   크롬은 **배경 탭**(과 가려진 창)의 `setTimeout` 을 **1초**로 묶는다.
   이 점검은 폴 한 번이 25ms 라고 보고 짜였으므로 그 자리에서 **40배**가
   되고, 실제로 자가시험 한 갈래가 19초 대신 **10~12분**이 걸렸다.
   11-3 이 「탭이 보이는 상태여야 합니다」 라고 이미 적어 둔 것인데,
   **점검이 그것을 말해 주지 않아** 사람이 30분을 기다리고도 까닭을
   몰랐다.

   **`visibilityState` 만 보지 않는다** — 창이 다른 창에 가려도 묶이고
   그때는 `visible` 이다. **실제로 한 번 재는 것**이 답이다.
   상한을 줄이지 않는다 — 재고 말하기만 한다. */
const 느린가 = async () => {
  if (묶임잰것 !== null) return 묶임잰것;
  const t = Date.now();
  await 잠깐잔다(25);
  const 걸림 = Date.now() - t;
  묶임잰것 = 걸림 > 200
    ? `타이머가 묶여 있습니다 (25ms 가 실제로 ${걸림}ms · 화면 ${document.visibilityState})`
      + ' — 탭을 앞으로 내고 다시 돌리세요. 결과는 맞지만 수십 배 느립니다'
    : '';
  return 묶임잰것;
};

const 점검 = async (옵션 = {}) => {
  /* `옵션.멈춤` — 그 말이 들어간 ✗ 가 나오면 **거기서 멈춘다.**

     자가시험이 고장마다 점검을 통째로 다시 돌아서 한 화면에 수십 분이
     걸렸다(2026-09-24 판이 목록 40분 · 달력 24분을 돌고도 못 끝냈다).
     고장 하나는 **한 항목**을 걸리게 하려고 심는 것이라, 그 항목이 ✗ 가
     된 뒤의 나머지는 그 고장에 대해 아무것도 말해 주지 않는다.

     **평소 점검은 이 옵션 없이 돈다** — 끝까지 간다. */
  ...
  const 잰다 = (ok, 라벨, 까닭) => {
    ...
    // **찾던 ✗ 가 나오면 거기서 멈춘다** (자가시험만 쓴다 — 위의 까닭)
    if (!ok && 옵션.멈춤 && String(까닭 === undefined ? 라벨 : 까닭).includes(옵션.멈춤))
      throw 멈춤표;
    return ok;
  };
  ...
  /* **기다림을 하나씩 적어 둔다** (2026-09-24 · 지시문 3 · 봐둘것 BI-h).
     ...
     몇이 상한을 다 썼는가** 가 있어야 한다. 합이 실제 시간과 안 맞으면
     남는 시간이 다른 데서 나는 것이고, 그때는 **단정하지 않는다.**
     재기만 하고 아무것도 안 바꾼다 — 상한도 조건도 그대로다. */
  const 기다림 = [];
  // 자가시험은 한 판에 점검을 여러 번 돌린다 — 마지막 것만 두면 **고장을
  // 심은 판의 장부가 사라진다.** 판마다 하나씩 쌓아 둔다
  window.__기다림 = 기다림;
  (window.__기다림들 = window.__기다림들 || []).push(기다림);
  const 적어둔다 = (무엇, 시작, 다씀, 갈래, 폴) => {
    // **폴 수를 함께 적는다** — 배경 탭에서는 한 폴이 25ms 가 아니라 **1초**라
    // (아래 `느린가`), 폴 수가 있어야 「그 판이 왜 40배였나」 가 셈으로 풀린다
    기다림.push({무엇, ms: Date.now() - 시작, 다씀, 갈래, 폴});
  };


  /* **자국을 남기는 누름은 개발 서버에서만 한다** (2026-09-24 사람이 정함 ·
     10장 · 봐둘것 BI-j). 확인 요청은 행이 남고 **알림이 나가므로**, 운영에서
     점검을 돌리면 남의 알림함에 요청이 선다.

     **판단은 여기 한 곳이고 모르면 안 보내는 쪽**이다 — 「개발이면 보낸다」
     가 아니라 「개발이라고 **확인된 때만** 보낸다」 로 적는다. 주소를 못 읽는
     자리가 생기면 조용히 보내는 쪽으로 기우는 것이 아니라 건너뛴다. */
  const 개발서버인가 = (() => {
    try {
      // 8001 은 `scripts/devserve.bat` 의 포트와 한 쌍이다 — 거기가 바뀌면
      // 이 항목이 조용히 건너뛰어진다(안 보내는 쪽이라 위험하진 않고,
      // 건너뛴 줄이 포트를 말하므로 사람이 알아본다)
      return location.hostname === '127.0.0.1' && location.port === '8001';
  ...
  const 잠깐본다 = async (조건, 상한 = 1200) => {
    const 끝 = Date.now() + 상한, 시작 = Date.now();
    let 폴 = 0;
    for (;;) {
      let ok = false;
      try { ok = !!조건(); } catch (e) { ok = false; }
      // **상한까지 보고 그대로 돌려준다** — 「안 되는 것이 답인 자리」 라
      // 못쟀음으로 새면 안 된다(test18_w04). 장부만 곁들인다
      if (ok || Date.now() >= 끝) { 적어둔다('잠깐본다', 시작, !ok, '잠깐', 폴); return ok; }
      폴++;
      await sleep(25);
    }
  };
  ...
  const 될때까지 = async (무엇, 조건, 상한 = 상한기본, 적는다 = true) => {
    ...
    for (;;) {
      let ok = false;
      try { ok = !!조건(); } catch (e) { ok = false; }
      if (ok) { 적어둔다(무엇, 시작, false, '될때까지', 폴); return true; }
      if (Date.now() >= 끝) {
        적어둔다(무엇, 시작, true, '될때까지', 폴);
        if (적는다) 못쟀음.push(무엇); return false;
      }
      폴++;
      await sleep(25);
    }
  ...
  /* **머리에서 잰다** — 글이 「시작할 때 한 번」 이라고 적었는데 끝에서
     재고 있었다(커밋 전 검토). 결과에 싣는 것은 끝이지만 **재는 것은
     여기**다. */
  const 묶임 = await 느린가();
  ...
  /* **완료 접힘은 아무것도 누르기 전에 잰다** (2026-09-25 사람이 정함 ·
     봐둘것 BI-i). 아래 그 항목은 `done.open === 안에펼침` 인데, **점검이
     행을 하나 펴고 나면 그 등식이 고장이 있든 없든 참**이 되는 계정이
     있다 — 보이는 줄이 전부 완료 안일 때(테스트리더가 그렇다) 행을 열면
     `open` 도 `안에펼침` 도 함께 참이 된다. 그래서 심은 고장이 정상과
     구별되지 않았고, 「점검이 못 가르는 자리」 가 놓침으로 세어졌다.

     **뒷정리보다도 먼저 잰다** — 그것도 누르는 일이라(`Drawer.close()`)
     `안에펼침` 을 바꾼다. `?task=` 로 들어와 열린 채인 것은 **고장이
     아니라 4-14 가 정한 모양**이고, 그때는 둘 다 참이라 그대로 지나간다. */
  const 첫완료접힘 = (() => {
    const d = document.querySelector('details.ldone');
    return d ? {open: d.open, 안에펼침: !!d.querySelector('.lslot:not([hidden])')} : null;
  })();
  ...
  const openers = () => isBoard
    ? (() => {
        const 전부 = [...sheet.querySelectorAll('.bar[data-run]')]
          .filter(b => shown(b) && !b.dataset.ghost);
        /* **내 부서 바를 앞세운다** (2026-09-24 · 목록과 같은 까닭).
           보드는 스스로 소속 필터를 「전체」 로 두므로 부서 리더로 돌리면
           남의 부서 바가 먼저 와서 **드로어 안 편집 항목 아홉이 건너뜀**이
           된다. 내 키는 부서 칸이(`data-mykeys`), 행은 자기 부서 키를
           (`data-of`) 이미 들고 있다 — 새로 알아낼 것이 없다.
           관리자는 부서 줄이 없을 수 있고, 그때는 순서를 안 바꾼다
           (어차피 전부 고칠 수 있다). */
        const 내키 = new Set((((document.getElementById('me') || {}).dataset
          || {}).mykeys || '').split(',').filter(Boolean));
        if (!내키.size) return 전부;
        const 내것 = 전부.filter(b =>
          내키.has((((b.closest('.row') || {}).dataset) || {}).of));
        return 내것.length
          ? [...내것, ...전부.filter(b => !내것.includes(b))]
          : 전부;
      })()
    : isList
    ...
    ? (() => {
        const 전부 = [...document.querySelectorAll('.trow.lrow')].filter(shown);
        /* **고칠 수 있는 행을 앞세운다** (2026-09-24 에 재어 보고 넣었다).
           부서를 전체로 두면 남의 부서 행이 먼저 와서 **드로어 안 편집 항목
           아홉이 통째로 건너뜀**이 된다. 그렇다고 「내 부서」 로 두면 **못
           바꾸는 상태 칸이 아예 없어** 그 두 쌍이 안 재진다 — 2026-09-08 부터
           한 번도 안 재진 자리가 그것이다.
           둘은 서로를 안 가린다: 상태 칸 시험은 `.cell.st.pick` 과
           `.cell.st:not(.pick)` 을 페이지에서 **따로** 찾는다. */
        const 고칠수있는 = 전부.filter(r => r.querySelector('.cell.st.pick'));
        return 고칠수있는.length
          ? [...고칠수있는, ...전부.filter(r => !고칠수있는.includes(r))]
          : 전부;
      })()
  ...
  let 완주 = false;
  /* **멈춘 판과 끝까지 돈 판을 가른다** (2026-09-24 검토) — 멈춤 줄은
     `·` 로 시작해 건너뜀 수에 섞여 들어가고, `완주` 는 둘 다 true 라
     표만 보면 어디까지 쟀는지 알 수 없다. 자가시험은 이 값이 참일 때만
     「그 자리에서 멈췄다」 로 읽는다. */
  let 멈춤함 = false;
  /* **점검이 사람의 드로어 폭을 덮는다** (2026-09-24 사람이 정함 · BI-j).
     아래 「폭 조절 핸들에서 눌러 여백에서 뗌」 이 `#grip` 을 끌고 `mouseup`
     을 쏘는데, 앱은 그 핸들러를 **window 에** 걸어 두고 `폭을쓴다(…)` 를
     기본값(`남길까 = true`)으로 불러 **localStorage 에 쓴다.**
     조용하고, 되돌리는 길이 없고, **실제로 다음 판을 망쳤다** — 그 값이
     최대로 남아 「달력 칸 크기 고정」 이 같은 값끼리 견줘 헛통과했다.

     그래서 **시작할 때 기억해 두고 끝에 되돌린다.** 값이 **없던 것도
     값이다** — 없었으면 지워야지 기본값을 새로 써 넣으면 그것도 자국이다. */
  const 폭키 = 'dcb.drawerWidth';
  let 폭처음 = null, 폭읽기실패 = false;
  try { 폭처음 = localStorage.getItem(폭키); }
  catch (e) { 폭읽기실패 = true; }
  const 폭되돌리기 = () => {
    if (폭읽기실패) return '처음 값을 못 읽어 되돌리지 않음';
    try {
      if (폭처음 === null) localStorage.removeItem(폭키);
      else localStorage.setItem(폭키, 폭처음);
      const 지금 = localStorage.getItem(폭키);
      return 지금 === 폭처음 ? '' : `되돌렸는데 값이 다름 (${폭처음} → ${지금})`;
    } catch (e) { return '되돌리지 못함: ' + ((e && e.message) || e); }
  };
  try {
      ...
      {until: () => shown($('p-review'))});
    {
      ...
      if (canEdit && firstDept && !개발서버인가) {
        // **운영에서는 안 보낸다** — 이 자리가 유일하게 자국을 남기는 누름이다
        results.push('· 확인 요청 보내기 — 건너뜀 (운영에서는 안 보냄 ·'
          + ' 개발 서버 127.0.0.1:8001 에서만 돈다)');
      } else if (canEdit && firstDept) {
    ...
    /* **먼저 좁혀 놓고 넓힌다** (2026-09-24 에 재어 보고 넣었다).
       기기에 남은 폭이 이미 최대(760)면 둘이 한꺼번에 샜다 —
       ㄱ. 「넓히기」 가 760→760 이라 **못 쟀음**으로 새고,
       ㄴ. 그보다 나쁘게, `narrow` 와 `wide` 를 **둘 다 넓은 상태에서** 재어
           `narrow === wide` 가 **같은 값끼리 견줘 헛통과**한다.
       그 값을 남긴 것이 앞 판의 이 점검 자신이었다(봐둘것 BI-g 와 같은 결). */
    document.documentElement.style.setProperty('--dw', '400px');
    await 될때까지('드로어 좁히기 (넓히기 전)', () => 드폭() < 500, 2000);
    const narrow = cellW();
    const 좁을때폭 = 드폭();
    ...
    잰다(narrow === wide, ` 달력 칸 크기 고정 (${narrow}px → ${wide}px)`,
      '드로어를 넓히면 달력이 커진다');
    /* 인라인 값을 걷는다. **다만 「자국을 안 남긴다」 고 적지 않는다** —
       그건 사실이 아니다(2026-09-24 커밋 전 검토가 짚었다):
       ㄱ. 이 자리에 오기 전에 **폭 조절 핸들을 끄는 시험**이 이미 돌았고,
           앱의 `mouseup` 핸들러가 `폭을쓴다(…)` 를 기본값(`남길까 = true`)
           으로 불러 **사람의 폭을 localStorage 에 덮는다.**
       ㄲ. 그래서 걷어도 돌아오는 것은 저장해 둔 폭이 아니라 CSS 의
           기본값이다(다시 읽거나 창 크기가 바뀌어야 앉는다).
       **그것이 「점검이 자기 다음 판을 못 재게 만든다」 의 뿌리**이고,
       위의 「먼저 좁혀 놓고 넓히기」 는 증상 쪽이다 — 봐둘것 BI-g. */
    ...
    if (isBoard) {
      ...
      if (done) {
        ...
        /* **여기서 다시 재지 않는다** — 이 줄에 닿을 때쯤이면 점검이
           이미 행을 열었다 (2026-09-25 사람이 정함). 들고 온 값은 맨 위에서
           **아무것도 누르기 전에** 잰 것이다. */
        if (!첫완료접힘) {
          results.push('· 완료 접힘은 첫 화면에 그 자리가 없어 건너뜀');
        } else {
          const {open: 첫open, 안에펼침: 첫안} = 첫완료접힘;
          잰다(첫open === 첫안,
            ` 완료는 기본 접힘 (첫 화면 open=${첫open} · 안에 펼친 업무 ${첫안 ? '있음' : '없음'})`,
            '완료 접힘이 기본 상태와 다름');
        }
    ...
    /* **일부러 멈춘 것과 죽은 것은 다르다.** 찾던 ✗ 를 만나 멈춘 것은
       고장이 아니라 그 고장이 잡혔다는 뜻이다 — 「끝까지 못 돌았다」 로
       세면 자가시험이 자기 신호를 고장으로 읽는다. */
    // **이 줄에 ✗ 를 쓰지 않는다** — ✗ 를 찍는 자리는 `잰다` 하나여야 하고
    // (`test20_p01`), 「자가시험」 이라는 말도 평소 점검 안에 있으면 안 된다
    // (`test19_s06`). 둘 다 이 판이 밟았다
    if (e === 멈춤표) {
      완주 = true; 멈춤함 = true;
      results.push('· 찾던 자리에서 멈춤 (고장 심기)');
    }
    else 잰다(false, ` 점검이 끝까지 돌지 못함 (${(e && e.message) || e})`,
      '점검이 중간에 죽었다: ' + ((e && e.message) || e));
  } finally {
    /* **점검이 중간에 죽어도 사람의 폭은 되돌아가야 한다** — 그래서
       `finally` 다(2026-09-24 사람이 정함). **실패를 삼키지 않는다**:
       조용히 넘어가면 다음 판이 또 남의 값 위에서 재게 된다. */
    const 폭탈 = 폭되돌리기();
    if (폭탈) 잰다(false, ` 드로어 폭을 되돌리지 못함 (${폭탈})`,
      '드로어 폭을 되돌리지 못함: ' + 폭탈);
  }
  ...
  /* **점검은 자기 자국을 걷고 끝난다** (10장 · 2026-09-25 에 재면서 나왔다).
     목록에서 행을 펴면 `details.ldone` 이 함께 열리는데, 그대로 두면
     **다음에 붙여넣는 판의 첫 화면**이 `open=true · 안에 펼친 업무 없음`
     이라 ✗ 로 시작한다 — 실제로 자가시험을 두 번 이어 돌리자 둘째 판이
     「심기 전부터 ✗ 가 있습니다」 로 멈췄다. 첫 화면 값은 이 함수 **맨
     위에서** 이미 쟀으므로 여기서 걷는 것이 그 판정을 가리지 않는다. */
  완료칸을걷는다();
...
/* **갈래가 기다리는 답은 두 가지다** (2026-09-25 사람이 정함).

   | 칸 | 맞는 답 | 쓰는 갈래 |
   |---|---|---|
   | `나와야` | 그 말이 든 **✗ 가 난다** | 대부분 |
   | `건너뜀나와야` | 그 말이 든 **`·` 줄이 나고 ✗ 는 안 는다** | 건너뛰는 길 |
   | `끝상태: '안빨감'` | **아무것도 안 빨갛고 뒷정리도 깨끗하다** | ㉠ |
   | `끝상태: '뒷정리실패'` | **`뒷정리` 가 못 닫는다** | ㉢ |

   **뒤의 둘은 `뒷정리` 자체를 재는 갈래다.** 이것이 없으면 누가 `뒷정리`
   의 닫는 줄을 지워도 자가시험이 조용히 초록이다 — 11-3 이 「심기를
   남기지 않으면 다음 입구는 못 잡는다」 고 적어 둔 그 자리이고, 앞 판이
   손으로만 만들어 재고 남기지 않아 실제로 그 상태였다.

   **갈래마다 넷 중 하나만 갖는다** — 둘을 함께 주면 어느 쪽으로 판정할지
   가 갈린다(`test19_s01` 이 그것을 잰다). */
const 고장들 = [
  ...
  {갈래: '⑨ 목록 — 완료 접힘', 고장: '완료가 기본으로 펼쳐져 있다',
   ...
   심는다: () => {
     const done = document.querySelector('details.ldone');
     const 원래 = done.open;
     /* **뒤집지 않고 등식을 깨는 값으로 못박는다** (2026-09-25 에 재면서
        나왔다). `!done.open` 으로 뒤집으면 **앞 갈래가 열어 두고 끝냈을 때**
        이 심기가 그것을 닫아, `open` 도 `안에펼침` 도 거짓이 되어 등식이
        참이 된다 — 심었는데 고장이 아닌 것이 되고 표에는 「놓침」 으로
        찍힌다. 재는 항목이 `open === 안에펼침` 이므로 **그 반대**를 넣는다. */
     done.open = !done.querySelector('.lslot:not([hidden])');
     return () => { done.open = 원래; };
   }},
  ...
  /* **막는 코드를 고친 판에는 그 막는 쪽 시험이 함께 있어야 한다** (11-3).
     2026-09-24 에 불투명 판정을 `color(srgb …)` 까지 읽게 넓혔는데, 그
     판정에는 심는 고장이 없어 「넓히다 반대쪽으로 새는 것」 을 못 잡았다
     (앞 판 검토가 짚었다). 왼쪽 라벨 열을 **진짜로 반투명하게** 만들어
     그 판정이 ✗ 를 내는지 본다 — 안 내면 그 판정이 새는 것이다. */
  {갈래: '㉑ 왼쪽 라벨 열이 반투명', 고장: '그 칸을 반투명하게 칠한다',
   화면: '보드',
   나와야: '왼쪽 라벨 열 배경이 반투명',
   심는다: () => {
     const st = document.createElement('style');
     st.textContent = '.sheet .row.main .lc,.sheet .row.sub .lc' +
  ...
  /* ── `뒷정리` 자체를 재는 둘 (2026-09-25 사람이 정함) ───────────────
     앞의 갈래들은 **점검이 무엇을 보는가**를 재고, 이 둘은 **갈래가 끝난
     뒤 치우는 길**을 잰다. 그래서 답이 ✗ 가 아니라 끝 상태다. */
  {갈래: '㉠ 연 채 끝나는 갈래', 고장: '갈래가 끝나며 드로어를 연다',
   끝상태: '안빨감',
   심는다: () => {
     /* **갈래가 끝나며 여는 것이 이 고장이다.** 점검이 도는 동안은
        아무 고장도 없고(그래서 판이 깨끗해야 한다), 끝나는 자리에서
        드로어를 연다 — 루프가 `끝낸다()` 다음에 `뒷정리()` 를 부르므로
        그 사이가 정확히 「연 채 끝난」 자리다. */
     let 열렸나 = false;
     return {
       끝낸다: () => {
         const 열개 = document.querySelector('.bar[data-run], .cal-dot[data-run], .trow.lrow');
         if (열개) 열개.click();
         열렸나 = !!(document.getElementById('drawer') || {}).classList
           && document.getElementById('drawer').classList.contains('open');
       },
       되돌린다: () => {},
       /* **안 열렸으면 「걸림」 이 아니다** (2026-09-25 커밋 전 검토).
          누를 것이 없거나 이미 열린 것을 눌러 **접혀** 버리면 뒷정리가
          닫을 것이 없어 `뒷정리탈` 이 비고, 그러면 「열린 것을 뒷정리가
          닫았다」 와 「열린 것이 없었다」 가 **같은 답**이 된다 — 11-3 의
          「센 것이 0 이면 성공이 아니라 실패」 그 축이다. */
       쟀나: () => 열렸나,
     };
   }},
  {갈래: '㉢ 닫는 길이 부서짐', 고장: 'Drawer.close 가 아무 일도 안 한다',
   끝상태: '뒷정리실패',
   심는다: () => {
     let 원래 = null, 막는다 = null;
     return {
       끝낸다: () => {
         원래 = window.Drawer && window.Drawer.close;
         if (!원래) return;
         /* **드로어를 열어 두고 닫는 길을 막는다.** 지금 `뒷정리` 가
            드로어를 닫는 길은 `Drawer.close()` **하나뿐**이고, 닫는
            단추를 막는 것은 그 길이 늘었을 때를 위한 덧방어다 — 주석이
            「나머지 길이 있다」 고 적혀 있어 없는 길을 찾게 만들던 자리를
            고쳤다(2026-09-25 커밋 전 검토). */
         const 열개 = document.querySelector('.bar[data-run], .cal-dot[data-run], .trow.lrow');
         if (열개) 열개.click();
         window.Drawer.close = () => {};
         막는다 = e => {
           if (e.target instanceof Element && e.target.closest('#drawer .dclose')) {
             e.stopImmediatePropagation();
           }
         };
         document.addEventListener('click', 막는다, true);
       ...
       되돌린다: () => {
         if (원래) window.Drawer.close = 원래;
         if (막는다) document.removeEventListener('click', 막는다, true);
       },
     };
   }},
];
...
/* **점검이 편 「완료 N건 보기」 를 걷는다** — 두 곳이 쓴다(`뒷정리` 와
   점검의 맨 끝). 두 벌로 적으면 조건이 갈리고, 갈린 쪽을 아무도 눈치채지
   못한다. **안에 펼친 것이 없을 때만** 닫는다 — 사람이 `?task=` 로 열어
   둔 것은 그 칸이 열려 있는 것이 맞는 모양이다(4-14). */
const 완료칸을걷는다 = () => {
  const 칸 = document.querySelector('details.ldone');
  if (칸 && 칸.open && !칸.querySelector('.lslot:not([hidden])')) 칸.open = false;
};
...
const 뒷정리 = async () => {
  ...
  /* ③-b **점검이 편 「완료 N건 보기」 도 닫는다** (2026-09-25 에 재면서
     나왔다). 목록에서 행을 펴면 그 행이 `details.ldone` 안일 때 그 칸이
     함께 열리는데, 드로어를 닫아도 **그 칸은 열린 채로 남는다.** 그러면
     다음 갈래의 첫 화면이 `open=true · 안에 펼친 업무 없음` 이 되어
     **「완료 접힘이 기본 상태와 다름」 으로 빨개진다** — 실제로 그것 하나가
     ㉠ 과 ⑦ 을 함께 놓침으로 만들고 맨 끝 판을 무효로 만들었다.
     **안에 펼친 것이 없을 때만 닫는다** — 사람이 `?task=` 로 열어 둔 것은
     그 칸이 열려 있는 것이 맞는 모양이다(4-14). 첫 화면 값은 이 함수보다
     **앞에서** 재므로 여기서 닫는 것이 그 판정을 가리지 않는다. */
  완료칸을걷는다();
...
const 자가시험 = async () => {
  ...
  /* **묶였는지 먼저 말한다** (2026-09-24 · BI-h). 끝에 말하면 **열두 분을
     기다린 뒤에** 안다 — 그때는 이미 판이 끝나 있다. 창에 먼저 두고
     나온것에도 싣는다. 막지는 않는다: 결과는 맞고 느릴 뿐이라,
     여기서 멈추면 「느린 기계에서는 아예 못 돌린다」 가 된다. */
  /* **깨끗한 판을 기다리지 않고 먼저 잰다** — 그 판이 묶인 탭에서 수백 초라,
     끝나고 말하면 **그 수백 초를 다 기다린 뒤에** 안다(커밋 전 검토). */
  const 묶임먼저 = await 느린가();
  if (묶임먼저) console.warn('[자가시험] ' + 묶임먼저);
  ...
  /* **어디까지 갔는지 창에 남긴다** (2026-09-24). 표는 맨 끝에 한 번
     ...
     갈래에서 멈췄는지 알 길이 없어 30분을 기다리고도 빈손이었다.
  ...
  for (const g of 돌갈래) {
    ...
    if (못심는까닭) {
      못심은갈래++;
      // **못 심은 것은 잡았다도 놓쳤다도 아니다** — 칸을 비워 두고 따로 센다
      표.push({갈래: g.갈래, 심은고장: g.고장, 심음: 못심는까닭,
               걸렸나: null, 못잰수: null, 실패수: null, 걸린초: 0});
      window.__진행.지난갈래.push(g.갈래 + ' (못 심음)');
      window.__진행.지금 = null;
      continue;
    }
    /* **그 고장이 걸려야 할 항목까지만 돈다.** 짝은 `나와야` 하나가 들고
       있다 — 표를 따로 두면 둘이 갈리고, 갈린 쪽을 아무도 눈치채지 못한다.
       `건너뜀나와야`/`완주못함` 갈래는 **끝까지 봐야** 판정이 서므로 안 건다. */
    const 멈춤 = (g.건너뜀나와야 || g.완주못함 || g.끝상태) ? undefined : g.나와야;
    /* **끝 상태를 재는 갈래는 심는 것과 되돌리는 것이 갈린다**
       (2026-09-25 사람이 정함). 그 둘의 고장은 **갈래가 끝나는 자리**에
       생기므로 점검이 도는 동안은 아무것도 없고, `뒷정리` 가 그것을
       만난 뒤에야 되돌린다 — 앞서 되돌리면 닫는 길이 살아나 ㉢ 이
       늘 성공한다. */
    const 끝갈래 = !!g.끝상태;
    const 심은것 = g.심는다(깨끗.항목);
    const 걷어낸다 = 끝갈래 ? 심은것.끝낸다 : 심은것;
    let 판;
    const 잰때 = Date.now();
    let 뒷정리탈 = [];
    ...
    뒷정리탈 = await 뒷정리();
    /* **㉢ 은 뒷정리가 실패하는 것이 정답이라 그 실패를 `뒷정리못함` 에
       안 싣는다** — 싣는 것은 **되돌린 뒤에도 안 닫힌 것**이다. 그것이
       진짜로 다음 갈래를 흔드는 자리다. */
    let 남은것 = 뒷정리탈;
    if (끝갈래) { 심은것.되돌린다(); 남은것 = await 뒷정리(); }
    if (남은것.length) 뒷정리못함.push({갈래: g.갈래, 남은것});
    const 실패 = (판.실패 || []).map(String);
    /* **건너뜀 갈래는 ✗ 가 아닌 것이 정답이다.** 그 `·` 줄이 나오고
       ✗ 가 하나도 안 늘어야 잡은 것이다 — 심었는데 ✗ 가 나면 그쪽이
       고장이다. */
    const 잡혔나 = g.끝상태 === '안빨감'
      /* ㉠ — **아무것도 안 빨갛고 뒷정리도 깨끗한 것**이 정답이다.
         뒷정리가 그 드로어를 못 닫으면 잡은 것이 아니다. */
      ? !실패.length && !판.치명 && !뒷정리탈.length
        /* **정말 열렸다 확인된 때만** 걸린 것으로 센다 — 안 열렸으면
           뒷정리가 닫을 것이 없어 저 셋이 저절로 참이 된다(커밋 전 검토) */
        && (!심은것.쟀나 || 심은것.쟀나())
      : g.끝상태 === '뒷정리실패'
      /* ㉢ — **뒷정리가 못 닫는 것**이 정답이다. 닫아 버리면 그 검사가
         새는 것이고, 그것이 이 갈래를 둔 까닭이다. */
      ? 뒷정리탈.length > 0
      : g.건너뜀나와야
    ...
    if (!잡혔나) 놓친갈래++;
    /* **한 갈래는 한 줄이고 칸은 「심음 → 걸린 항목 · 걸렸나」 다**
       (2026-09-24 사람이 정함). 전에는 「안 심음」 칸이 함께 있었는데,
       그 칸은 **갈래마다 다시 재는 것이 아니라 맨 위 깨끗한 판을 옮겨
       적은 것**이라 늘 같은 값이었다 — 두 칸처럼 보이지만 잰 것은 하나다
       (앞 판 커밋 전 검토가 짚었고 사람이 걷기로 정했다). 안 심었을 때
       깨끗한지는 이제 **맨 처음과 맨 끝의 깨끗한 판**이 말한다.

       **짝(고장 ↔ 걸려야 할 항목)은 `고장들` 의 `나와야` 하나에만 둔다** —
       표에 그것을 또 실으면 두 벌이 되고, 갈린 쪽을 아무도 눈치채지 못한다.
       표가 내는 것은 **무엇이 나왔나**이지 무엇이 나왔어야 하나가 아니다. */
    /* **「못 잰 수」 와 「실패 수」 를 함께 싣는다** (2026-09-24 사람이 정함).
       앞 판이 걷었던 자리인데, 걷은 까닭은 그 판 지시문이 「심음 → 걸린
       항목 · 걸렸나만」 으로 **좁게 적혀서**였다. 그런데 `못잰수` 는
       BI-h 의 물음(「왜 한 갈래가 10~12분인가」)을 **바로 가르는 수**다 —
       못쟀음 한 줄이 곧 상한을 다 쓴 기다림 한 번이라, 그 수가 이백 언저리면
       「기다림이 상한을 다 쓴다」 가 맞고 스물쯤이면 틀리다. 그 칸을 지운
       판에서 그 물음을 열었다(앞 판 커밋 전 검토). */
    표.push({갈래: g.갈래, 심은고장: g.고장,
             심음: 잡혔나
               ? (g.끝상태 === '안빨감' ? '안 빨감 — 걸림'
                  : g.끝상태 === '뒷정리실패' ? '뒷정리 실패 — 걸림'
                  : g.건너뜀나와야 ? '건너뜀 — 걸림' : '✗ — 걸림')
               : g.끝상태 === '안빨감' ? '빨갛거나 못 닫음 — 검사가 새는 입구다'
               : g.끝상태 === '뒷정리실패' ? '뒷정리가 닫아 버림 — 검사가 새는 입구다'
             ...
             실패수: 실패.length,
             걸린초,
             /* **`완주` 만 보면 멈춘 판과 끝까지 돈 판이 같아 보인다**
                (2026-09-24 검토) — 멈춘 것은 「그 자리를 찾았다」 는 뜻이라
                `완주: true` 인데, 나머지 항목은 안 잰 것이다. 따로 적는다. */
             완주: 판.완주, 멈췄나: !!판.멈춤함});
    // **진행 표시에도 두 수를 싣는다** — 안 끝나도 읽는 자리라(그것이 이
    // 표시를 둔 까닭이다) 여기 없으면 판이 안 끝났을 때 BI-h 를 못 가른다
    window.__진행.지난갈래.push(g.갈래 + (잡혔나 ? ' (걸림)' : ' (놓침)')
      + ' · ' + 걸린초 + '초 · 못잼 ' + (판.못쟀음 || []).length
      + ' · 실패 ' + 실패.length);
    window.__진행.지금 = null;
  }
  window.__진행.지금 = {갈래: '맨 끝 깨끗한 판', 든때: Date.now()};
  /* **맨 끝에 깨끗한 판을 한 번 더 잰다** (2026-09-24 사람이 정함).
     갈래마다 `finally` 로 걷어내지만 **걷혔는지는 아무도 안 봤다** — 하나가
     안 걷히면 그다음 갈래의 판정이 통째로 흔들리고, 그때 나오는 것은
     「놓쳤다」 라서 **원인이 심기 탓인지 점검 탓인지 안 갈린다**(봐둘것 BI-g).
     맨 끝 판이 실패 0 이 아니면 **이 판의 판정 전체가 무효다** — 갈래별
  ...
  return {자가시험: 다걷혔나 && 놓친갈래 === 0 && 잰갈래 > 0,
          ...
          // **두 판을 나란히 낸다** — 맨 처음과 맨 끝이 같아야 갈래별
          // 줄을 믿을 수 있다
          깨끗한판: {맨처음: {실패: 깨끗.실패.length, 항목수: 깨끗.항목수,
                           건너뜀: 깨끗.건너뜀, 못쟀음: 깨끗.못쟀음.length},
                   맨끝: {실패: (끝깨끗.실패 || []).length,
                         항목수: 끝깨끗.항목수, 건너뜀: 끝깨끗.건너뜀,
                         못쟀음: (끝깨끗.못쟀음 || []).length,
                         치명: 끝깨끗.치명}}};
};
