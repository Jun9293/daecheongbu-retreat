 ...
 */
(async () => {
/* **본문을 `main` 하나로만 찾지 않는다** (2026-09-17 · 봐둘것 BE-a). 보드·달력에 `main` 이
   없던 동안 두 화면이 거짓으로 실패했고, 그 탓에 본문 누름이 한 번도 안 재졌다.
   `main` 이 먼저이고, 없으면 그 화면의 본문 자리를 차례로 본다 — 무엇을 잡았는지 라벨에 적는다. */
const 본문후보 = ['main', '#mlist', '#calswap'];
const 점검 = async () => {
  ...
  const 고른 = 본문후보.find(q => document.querySelector(q));
  const 본문 = 고른 ? document.querySelector(고른) : null;
  잰다(!!(본문 && 본문.textContent.trim().length > 20 && 화면안(본문)),
       ` 본문이 그려졌다 (${고른 || '본문 자리 없음'})`, '본문이 비어 있거나 화면 밖이다');
...
const 고장들 = [
  ...
  {갈래: '⑥ 본문의 링크가 덮여 있다',
   ...
   심는다: () => 스타일(본문후보.map(q => q + ' a').join(',') + '{pointer-events:none}')},
  {갈래: '⑦ 단추의 두 이름이 갈린다',
