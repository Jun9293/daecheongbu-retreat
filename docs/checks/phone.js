...
const 점검 = async () => {
  ...
  /* **열림과 닫힘은 서로의 반대가 아니다.** 사이드바는 0.18초 동안
     ...
     간다**: 실제로 그 때문에 「목록 단추가 무언가에 덮여 있다」 가 떴고,
  ...
  잰다(누를수있나(toggle), ' 목록 단추가 눌리는 자리에 있다 (무엇에도 안 덮여 있다)',
       '목록 단추 자리에서 누름을 받는 것이 그 단추가 아니다 — 무언가 위를 덮고 있다');

  toggle.click();
  const 열림 = await 될때까지('목록 단추를 눌러 사이드바가 열리는 것', 열렸나);
  /* **이 줄이 이 파일이 생긴 이유다.** 못 쟀음이 아니라 ✗ 다 — 「안 열린다」 는
     화면의 고장이고, 사람이 본 것이 정확히 이것이다. */
  잰다(열림, ' **목록 단추를 누르면 사이드바가 화면 안으로 들어온다**',
       '목록 단추를 눌러도 사이드바가 안 열린다 (휴대폰에서 아무 데도 못 간다)');
...
const 고장들 = [
  {갈래: '① 옛 고장 — 토글이 sidepin 만 붙인다',
   나와야: '목록 단추를 누르면 사이드바가 화면 안으로 들어온다',
   심는다: () => {
  ...
  {갈래: '⑤ 목록 단추가 누름을 안 받는다 (pointer-events)',
   나와야: '목록 단추가 눌리는 자리에 있다',
