...
_ADDED_COLUMNS: tuple[tuple[str, str, str], ...] = (
    ...
    # 업무 쪽의 같은 자리 (노션 업무를 그대로 들이는 판). NULL 로 붙고 부팅은 값을
    # 안 채운다 — 채우는 것은 scripts/노션업무들이기.py 다.
    ("task_library", "notion_page_id", "VARCHAR(36)"),
    ...
    # 예산 항목·수입 취소 표시 (7-3) — 지출과 같은 모양. 지우던 길을 걷었다 (0장).
    # 붙이기만 한다 — 옛 행은 NULL(산 행)이다.
    ("budget_categories", "canceled_at", "DATETIME"),
    ("income_items", "canceled_at", "DATETIME"),
...
def _warn_old_equipment_shape() -> None:
    ...
    고치는 것은 `scripts/비품묶음옮기기.py` 다 — 누가 돌리는지는 11-2 가 정한다.
    부팅이 표를
