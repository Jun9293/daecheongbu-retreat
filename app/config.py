...
#
# **숫자만 올리면 안 된다.** 큰 파일은 아래의 자리마다 같이 닿는다 —
...
#
# **제약은 시간이 아니라 크기입니다.** 처음에는 "30초를 넘기면 끊긴다,
# 2MB/s 이니 60MB 언저리" 로 적었는데, 그 측정은 **로그인하지 않은 요청**이라
# 서버가 본문을 다 받기 전에 로그인 화면으로 답해 버린 것일 수 있습니다.
# 실제로 걸리는 것은 Cloudflare 의 **요청 본문 100MB** 제한이고, 넘으면
# 파일이 서버에 닿기도 전에 **413** 이 돌아옵니다. 그 숫자를 시간으로
# 적어 두면 다음 사람이 상한을 60 으로 내립니다.
