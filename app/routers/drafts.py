...
총무팀이 초안을 열면 부서마다 칸이 하나씩 생긴다. **자기 부서 소속이면(리더·팀원
모두) 그 칸을 채우고 제출할 수 있다** — 제출도 소속이면 된다(2026-09-10 UI 정리
판 2 · 봐둘것 AM-e). 임시저장은 몇 번이든, 제출은 "우리 팀은 다 골랐다"는 표시다.
총무팀이 회차를 열기 전까지는 제출한 뒤에도 고칠 수 있다.
"""
