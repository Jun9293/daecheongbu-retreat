    ...
    **비어 있는 것이 세 가지 뜻을 갖지 않게 한다.** '아직 안 끝났다' ·
    ...
    for x in 것들:
        ...
        항목.append({
            ...
            # 이력이 갈린다 (6-2 · 12장)
            #
            # **지금 코드로는 늘 False 다.** `source_meeting_id` 를 붙이는
            # 곳은 `create_run` 하나이고 거기서는 늘 `included=True` 이며,
            # 회차를 연 뒤 업무를 **빼는 길이 아직 없다**. 그러니 이 값이
            # 참이 되려면 「이번 회차에서 빼기」 가 먼저 생겨야 한다 —
            # 그때 되살리기가 `relink_prerequisites` 를 함께 불러야 하는
            # 것까지 12장에 적어 두었다. 지금 지우지 않는 이유는, 세는
            # 범위(`made_from_meeting`)가 뺀 것도 세기 때문이다: 세면서
            # 화면이 아무 말도 안 하면 「이미 만들었습니다」 가 목록에
            # 없는 것을 가리킨다
