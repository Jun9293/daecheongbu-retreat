...
function renderMoved() {
  ...
  if (m.why === 'hand') {
    ...
    /* **아직 그 자리에 안 서 있을 수 있다.** 라이브러리를 고쳐도 그 자리에서
       run 은 안 움직이고 다음 번 개회일 변경에서야 옮겨진다 — 그때까지
       「따르고 있습니다」 라고 하면 바로 위 기간과 두 말을 한다. */
    글 = m.at_library === false
      ? `라이브러리 기준이 <b>그 뒤에 바뀌었습니다</b> — 지금 서 있는 곳은 `
        + `손으로 옮겨 둔 ${자리}이고, <b>다음에 개회일이 바뀌면</b> 라이브러리 자리를 따릅니다.`
      : `라이브러리 기준이 <b>그 뒤에 바뀌어</b> 그쪽을 따르고 있습니다 — `
