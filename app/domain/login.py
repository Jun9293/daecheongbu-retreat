...
# 바꾼 것을 기록에 적는 말. **화면이 둘이라 여기 한 번만 적는다** —
# 두 곳에 적으면 같은 일이 활동 기록에 두 이름으로 남는다.
바꾼_행위 = "비밀번호_변경"


def 바꾼_말(user: User) -> str:
    return f"{user.name} 님이 비밀번호를 바꿨습니다."


def 바꿔도되나(새값: str, 확인값: str) -> str | None:
    """못 바꾸는 사유 하나, 괜찮으면 None.

    **판정은 여기 하나다** — 첫 비밀번호를 바꾸는 화면(`/password`)과
    설정 › 내 정보가 같이 부른다. 두 벌이 되면 한쪽에서만 통과하는 값이
    생기고, **갈린 쪽을 아무도 눈치채지 못한다.**

    **지금 비밀번호는 묻지 않는다** — 사람이 그렇게 정했다(2026-09-11).
    그래서 남는 것과 그 까닭은 `docs/봐둘것.md` AS-a 에 적어 두었다.
    """
    if 새값 != 확인값:
        return "두 번 적은 값이 다릅니다."
    if 너무짧나(새값):
