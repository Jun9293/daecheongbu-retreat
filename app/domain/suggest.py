...
# **제안의 어휘는 여기 하나다.** 빼는 목록이 아니라 넣는 목록이다 —
# 빼는 목록이면 새 종류가 생길 때 자동으로 제안으로 세어진다. 실제로
# '더있음'(안내 줄)이 그렇게 들어와 표본 20개를 21개로 세었고,
# "성적표의 21개와 다르면 멈춘다" 검사가 **틀린 채로 통과했다.**
#
# 안내까지 함께 두는 이유는 어휘를 **완전하게** 만들기 위해서다 —
# 그래야 *둘 중 어디에도 없는 것* 이 곧 "모르는 종류" 가 되고,
# 그때 채점표에 찍지 않고 거절할 수 있다.
제안종류 = {"decision": "결정사항", "discussion": "논의", "new": "새 업무"}
안내종류 = {"더있음": "안내"}


@dataclass
class 제안:
    kind: str                       # `제안종류` 또는 `안내종류` 의 키
...
- **짧고 흐릿한 줄도 보세요.** 담당자만 적힌 줄("담당 : 아무개"), 물음으로
  ...
  확인해야 할 것**이면 냅니다. 문장이 짧다고 건너뛰지 마세요 — 실제로
  이런 줄에서 빠뜨린 것이 가장 많았습니다.
- **정해진 일은 논의로 그치지 말고 업무까지 내세요.** 「누가 · 무엇을 ·
  언제까지」 중 **둘 이상**이 정해졌으면 그것은 할 일입니다. 그 업무가
  목록에 있으면 `논의`로, 목록에 없으면 `새 업무`로 내세요.
  **하나의 대목에서 논의와 새 업무가 함께 나올 수 있습니다** — 목록의
  업무에 붙일 사정이면서 동시에 목록에 없는 할 일이 생겼다면 둘 다 내세요.
- **그 경계는 「누군가 무엇을 해야 하는가」 입니다.** 의견·소감·상황 설명·
  지나간 보고에는 업무를 붙이지 마세요. 할 일이 아닌 것을 업무로 내는 것도
  똑같이 틀린 것입니다.
...
def 쪽지(r: 결과) -> str:
    """`Meeting.suggest_note` 에 남길 한 줄 — `방식|말`.

    **쓰는 곳과 읽는 곳이 한 파일에 붙어 있어야 한다.** 형식이 바뀌면
    여기 둘만 고치면 된다.
    """
    return f"{r.방식}|{r.말}"


def 쪽지읽기(note: str | None) -> tuple[str, str]:
    """`suggest_note` 를 (방식, 말) 로. **읽는 곳은 여기 하나다.**

    전에는 이 형식을 아는 곳이 라우터에 흩어져 있었다 — 제안 화면·논의
    반영·새 업무 반영이 각자 쪼갰고, `낱말로_물러섰나` 는 앞머리를
    글자로 견줬으며, 쓰는 곳도 따로 이어 붙였다. 형식이 바뀌면 그 전부를
    고쳐야 했고, 한 곳만 고치면 **갈린 쪽을 아무도 눈치채지 못한다** —
    이 프로젝트가 가장 자주 고쳐 온 모양이다. (개수를 세어 적지 않는다 —
    10장. 어디가 부르는지는 `test13_n01` 이 잰다.)

    **구분자가 없으면 방식을 모르는 것이다.** 옛 쪽지·오류 문구가 그런
    모양인데, 앞부분을 방식으로 읽으면 화면이 「분석하지 못했습니다…
    로 골랐습니다」 라고 말하게 된다. 그때는 방식을 비우고 **글 전체를
    말로** 돌려준다 — 사람에게 보여줄 것은 남아 있어야 한다.
    """
    방식, _, 말 = (note or "").partition("|")
    if not 말:
        return "", 방식
    return 방식, 말


def 낱말로_물러섰나(note: str | None) -> bool:
    """지난번 결과가 낱말 겹침이었나 — 쪽지를 **읽는 규칙 하나**를 지난다."""
    return 쪽지읽기(note)[0] == "낱말"
