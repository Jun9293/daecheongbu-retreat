...
# **「기획」 과 「준비」 를 가르는 자리** (목업 D · 2026-09-23 사람이 정함) —
# D-16주보다 이른 주가 기획이다. 보드의 구간 줄이 이 하나로 갈린다.
PLANNING_UNTIL_D_WEEK = 16

# **후속 칸의 상한** — 수련회 다음 날부터 주 단위로 D+1, D+2 … 로 가고
# 여기까지다(목업 D · 2026-09-23 사람이 정함). 앞쪽의 `MAX_FIRST_WEEK` 와
# 같은 모양의 짝이다.
LAST_AFTER_WEEK = 26
# **후속은 적어도 이만큼은 그린다** — 마감으로만 정하면 폐회 직후의 회차에서
# 축이 한 칸에 멈춘다. 그러면 「오늘」 단추가 옮길 오늘 선이 축에 없다.
MIN_AFTER_WEEKS = 3
