...
그래서 **뜻을 붙이지 않는다.** 본문에는 `⟨형광펜⟩` 으로 넣는다.

처음에는 `⟨미완료?⟩` 로 넣었다. 물음표가 있으니 추측인 줄 알 것이라 여겼는데,
**물음표가 있어도 뜻을 주장한다** — 나중에 읽는 사람은 물음표를 안 읽고
"미완료" 만 읽는다. 근거가 2/20 인 읽기를 본문에 박아 두면, 그 읽기가 사실이
되어 돌아다닌다.

`⟨형광펜⟩` 은 **칠해져 있었다는 사실만** 말한다. 그건 확실하고, 원본의 정보도
그대로 지켜진다 (6-9 — 지어낸 기록을 심지 않는다). 뜻이 밝혀지면 그때 붙인다.
"""
...
def cut(text: str, *, source: str) -> tuple[list[회의], list[걸림]]:
    ...
    **조용히 지나가는 것을 만들지 않는다** (4-10 의 '빠진 선행' 과 같은 자리).
    ...
    ### 이미 본 것 — 다시 들여다보지 않아도 된다

    옮기기는 **일회성 이사**다. 2026 여름수련회 원본의 `날짜같은줄` 2건은
    사람이 이미 보았고 판정도 났다. **경고는 판정이 적히는 순간 제 몫을
    다한다** — 다음에 스크립트를 돌릴 때 같은 두 줄을 또 들여다볼 이유가 없다.
    ...
    둘 다 **지금 처리(자르지 않음)가 맞다.** 새 원본을 넣을 때 이 둘 말고
    다른 것이 뜨면 그때 보면 된다.
