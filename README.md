...
로그인은 **아이디와 비밀번호**로 합니다. 아이디는 **이름의 로마자**이고,
첫 비밀번호는 총무팀이 만들어 카카오톡으로 전달합니다. 받은 사람은 처음
들어올 때 비밀번호를 바꿉니다 — 바꾸기 전에는 다른 화면이 열리지 않습니다.
한 번 들어오면 그 기기에서 계속 로그인된 채로 남습니다.

(전화번호 SMS 인증도, 그다음의 초대 링크도 접었습니다 — 이유는 CLAUDE.md 4-12.
옛 초대 주소를 누르면 404 가 아니라 로그인 화면으로 갑니다.)
...
찍히는 아이디와 비밀번호로 들어가면 됩니다. 그다음부터는 `/admin/users`
화면에서 만듭니다. **비밀번호를 잊었으면 계정을 다시 만들지 말고** 비밀번호만
다시 받으세요 — 계정이 중복되면 알림이 그 수만큼 갑니다.

```powershell
.venv\Scripts\python.exe scripts\계정문열기.py
.venv\Scripts\python.exe scripts\계정문열기.py --재설정 <id>
```
...
| `/admin/users` | **계정 관리** — 아이디 정하기 · 첫 비밀번호 발급 |
...
| 이름 | 왜 |
|---|---|
| `DCB_BASE_URL` | 바깥에서 보이는 주소. 안내에 적히는 로그인 주소입니다 |
