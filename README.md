...
| 경로 | 무엇 |
|---|---|
| `/` | **홈** — 회차 헤더 · 경고 띠 · 지표 4칸 · 내 할 일 · 예산 집행 (4-15) |
| `/tasks` | **수련회 준비 — 목록** — 시작일 순. 행을 펼치면 보드와 같은 상세가 그 자리에서 열림 (4-14) |
| `/board` | **수련회 준비 — 보드** — D-주차 타임라인. 부서 → Main → 하위 트리, 상태·논의·첨부·진단 패널 |
...
| `/settings/library` | **업무 라이브러리** — 회차를 넘어 남는 업무. 필수 지정·선후행 (옛 `/library` 는 301) |
| `/notifications` | **알림** — 받은 것 / 내가 보낸 요청. 확인 요청의 승인·반려도 여기 (4-16) |
| `/budget` | **예산** — 구분 › 항목 › 세부항목 · 소계 · 수입 · 잔액 (7-3) |
| `/expenses` | **지출** — 예산 라인별 그룹 · 식대 정산 · 영수증 N개 · 환급 필터 (7-4) |
| `/draft` | **회차 준비** — 각 팀이 자기 칸을 채우고 제출 |
| `/admin/users` | **계정 관리** — 초대 링크 발급·재발급 |
| `/admin/notify/preview` | **알림 미리보기** — 오늘 누구에게 무엇이 갈지 (발송은 아직 안 켰습니다) |

UI 개편(2026-09)이 끝나 **옛 설계 화면은 남아 있지 않습니다** — 껍데기는
retreat_base.html 하나입니다 (CLAUDE.md 14장).
